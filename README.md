...
| Windows audio | Endpoint enumeration, bounded receive-only probes, and experimental event-driven shared-mode WASAPI capture into a fixed real-time queue; recording/placement is unavailable |
| MPC/DAW branches and merge | In-memory merge planning plus a crash-recoverable versioned canonical-session snapshot store; this is not yet a distributed commit graph |
| Desired/observed state, acknowledgement, clocks, recording selection and Hardware Learn | Core contracts plus qualified-mapping validation and write-intent planning; no real MPC mapping or DAW relay is enabled |
| Background service | Persistent per-user named-pipe service with current-user ACL, DPAPI-protected credential, protocol negotiation, bounded frames, durable status and clean/unclean restart evidence |
| Windows MIDI mediation core | Fixed-capacity routing queues, explicit clock/transport authorities, direction gates, echo suppression, rate limiting, queue saturation and synthetic concurrent stress coverage |
| Experimental VST3 client | Optional audio-transparent component/controller target; passes the Steinberg 3.8.1 validator but is unsigned and has no live service or host-certified integration |
| Hardware project write-back and live two-way parameter synchronization | Intentionally unavailable until their separate qualification gates pass |

The prototype uses a transparent exchange-package fallback because VST3 is a plug-in interface for real-time audio components rather than a universal permission to create proprietary DAW project files. The repository now contains an optional thin VST3 client and a pinned-SDK CI route, but each host integration must still be verified before it is enabled.[1] Reason supports VST3 plug-ins in its standalone music-making software, making the shared client a practical evaluation path, but that does not by itself establish project-writing or full session-control capability.[2]
...
├── src/plugins/vst3/              # Optional experimental, audio-transparent VST3 client
├── profiles/                      # Device and DAW capability declarations
├── release/release-gates.json     # Machine-checked commercial release decision record
├── evidence/                      # Scope-limited physical/toolchain observations
...
The standard ZIP and optional WiX MSI are pre-release artifacts. The MSI route is per-user and its
install/repair/uninstall lifecycle has been exercised locally, but it is not code-signed and is not
approved for commercial distribution. See [`packaging/windows/README.md`](packaging/windows/README.md).

The VST3 client is deliberately excluded from normal builds. Obtain and review the Steinberg SDK
separately, keep it outside tracked source, then explicitly configure the experimental target. The
validated reference revision is `v3.8.1_build_84`; this is a build/validator gate, not host certification.

```powershell
git clone --branch v3.8.1_build_84 --depth 1 --recurse-submodules --shallow-submodules `
  https://github.com/steinbergmedia/vst3sdk.git build/vendor/vst3sdk
cmake -S . -B build/vst3-msvc -G "Visual Studio 18 2026" -A x64 `
  -DUBRIDGE_BUILD_VST3_CLIENT=ON `
  -DUBRIDGE_VST3_SDK_ROOT="$pwd/build/vendor/vst3sdk" `
  -DUBRIDGE_WARNINGS_AS_ERRORS=ON
cmake --build build/vst3-msvc --config Release --target UniversalBridgeVST3 --parallel 4
```
...
Some current fingerprints and the durable-session envelope use FNV-1a 64-bit values to detect ordinary fixture/file corruption efficiently. They are **not cryptographic integrity or authenticity proofs**. Authenticated service credentials use Windows cryptographic randomness and DPAPI instead. Before production synchronization, content identity must move to a cryptographic hash (such as SHA-256 or BLAKE3), with source/destination revision vectors, cancellation recovery, and independently testable conflict-resolution policies.
...
A passing test suite confirms prototype behavior only. The deterministic stress target covers two million concurrent queue transfers and 250,000 synthetic mediated MIDI messages, but it is not a physical endpoint or long-duration soak. The optional VST3 target separately passes Steinberg's validator. None of this proves compatibility with a particular MPC firmware, Cubase or Reason edition, audio interface, USB hub, operating-system driver, or complete DAW session.

## Operating-system profiles

| Target OS | Hard-coded state | Current behavior |
|---|---|---|
| Windows | `reference_desktop` | Preflight/exchange, partial XPJ intake, experimental WinMM/WASAPI streaming, authenticated local IPC, and an optional validator-passing VST3 client; complete live mediation remains disabled |
...
The ten commercial release gates, exact current evidence and remaining exit criteria are maintained in
[`docs/COMMERCIAL_RELEASE_GATES.md`](docs/COMMERCIAL_RELEASE_GATES.md) and machine-checked from
[`release/release-gates.json`](release/release-gates.json). `python tools/check_release_readiness.py
--release` intentionally fails while any gate is not `ready`.
...
| Windows hardware service | Connect the authenticated service to service-owned physical endpoints and durable live sessions | Tested reconnect, multi-client mediation and permission behavior on the reference MPC Sample |
...
| Cubase/Reason host adapters | Connect the validator-passing VST3 client to authenticated non-real-time IPC | Verified per-host automation, crash isolation, capability matrix and reopen persistence |
